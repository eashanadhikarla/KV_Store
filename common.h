/* 
*	Required to install two things inside docker container before starting;
*	- Boost Libraries : apt-get install libboost-all-dev --fix-missing
*	- Google V8 JavaScript engine : apt-get install libv8-dev 
*/
...
void print_error(v8::TryCatch *error) {
	...
	std::cout << *file_name << ":" << line_number << ": " << *exception
	<< std::endl
	<< *source << std::endl;
}
...
/**
* Execute some JavaScript code. Also give that code a name, so we can try
* to run it again in the future, by name.
...
*/
bool run_code(v8::Handle<v8::String> code, v8::Handle<v8::Value> name,
...
*/
bool load_run_script(const std::string &script,
const v8::Handle<v8::Context> &context) {
	...
	std::string text = load_file(script.c_str());
	...
	if (!run_code(source, v8::String::New(script.c_str()), context))
		return false;
